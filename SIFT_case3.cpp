...
int nearestNeighbor(const Mat& vec, const vector<KeyPoint>& keypoints, const Mat& descriptors) {
	...
	for (int i = 0; i < descriptors.rows; i++) {
		...
		double temp = euclidDistance(vec, v);
		if (minDist > temp) {
			minDist = temp;
			neighbor = i;
		}
...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1,
	...
	for (int i = 0; i < descriptors1.rows; i++) {
		...
		if (nn < 0 || nn >= keypoints2.size()) {
			cout << "invalid match index: " << nn << " at i = " << i << endl;
			continue;
		}


		double minDist = 1e6, secondMinDist = 1e6;
		...
		if (ratio_threshold) {
			if (minDist / secondMinDist >= RATIO_THR) {
				continue;
			}
		...
		if (crossCheck) {
			...
			int reverseNN = nearestNeighbor(desc2, keypoints1, descriptors1);
			if (reverseNN != i) continue;
		}
